...
// For some odd reason...
// #define ntohl(x) ((unsigned long int)((((unsigned long int)(x) & 0x000000ffU) << 24) | (((unsigned long int)(x) & 0x0000ff00U) << 8) | (((unsigned long int)(x) & 0x00ff0000U) >> 8) | (((unsigned long int)(x) & 0xff000000U) >> 24)))
...
short unsigned int DOOMPORT = (IPPORT_USERRESERVED + 0x1d);
...
void PacketGet(void)
{
    ...
    doomcom->datalength = static_cast<size_t>(c);
...
char *GetLocalAddress(void)
{
    ...
    return hostentry->h_addr_list[0];
}
...
void I_InitNetwork(void)
{
    ...
    if (p && p < myargc - 1)
    {
        ...
        DOOMPORT = static_cast<short unsigned int>(atoi(myargv[p + 1]));
    ...
    while (++i < myargc && myargv[i][0] != '-')
    {
        ...
        else
        {
            ...
            sendaddress[doomcom->numnodes].sin_addr.s_addr = reinterpret_cast<in_addr_t>(hostentry->h_addr_list[0]);
        }
        doomcom->numnodes++;
    }
